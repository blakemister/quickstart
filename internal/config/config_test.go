...
func TestDefaultAccounts(t *testing.T) {
	if len(DefaultAccounts) != 6 {
		t.Fatalf("expected 6 default accounts, got %d", len(DefaultAccounts))
	}
...
func TestEnsureDefaultsSyncsStaleArgs(t *testing.T) {
	// Simulate a saved config where opencode had --yolo (which it never supported)
	cfg := &Config{
		Accounts: []Account{
			{ID: "opencode", Label: "OpenCode (z.ai)", Command: "opencode", Args: []string{"--yolo"}, Enabled: true},
		},
	}
	EnsureDefaults(cfg)

	oc := AccountByID(cfg.Accounts, "opencode")
	if oc == nil {
		t.Fatal("expected opencode account")
	}
	// Should have been synced to the current default (empty args)
	if len(oc.Args) != 0 {
		t.Errorf("expected opencode args to be empty after EnsureDefaults, got %v", oc.Args)
	}
}
