...
func runRoot(cmd *cobra.Command, args []string) error {
	...
	if m, ok := finalModel.(tui.PickerModel); ok && m.Err() != nil {
		return fmt.Errorf("tool exited with error: %w", m.Err())
	}
	return nil
}
...
func cleanupOldBinaries() {
	...
	cleanupOldBinariesIn(filepath.Dir(exe))
}

func cleanupOldBinariesIn(dir string) {
	matches, _ := filepath.Glob(filepath.Join(dir, "qs-old-*.exe"))
